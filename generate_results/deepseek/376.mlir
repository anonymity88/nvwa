...
module {
  ...
  func.func @main(
    ...
    %arg3: i32, 
    %arg4: i32,
    %input_complex: tensor<1x16x16x4xcomplex<f32>>,
    %filter_complex: tensor<3x3x4x16xcomplex<f32>>,
    %output_complex: tensor<1x14x14x16xcomplex<f32>>
  ) -> (tensor<64xf32>, tensor<1x10x8x8xf32>, tensor<?x?x?x?x1xi32>, tensor<1x14x14x16xcomplex<f32>>) {
    ...
    %complex_result = call @conv_complex(%input_complex, %filter_complex, %output_complex)
                     : (tensor<1x16x16x4xcomplex<f32>>, tensor<3x3x4x16xcomplex<f32>>, tensor<1x14x14x16xcomplex<f32>>) -> tensor<1x14x14x16xcomplex<f32>>
    
    return %map_result, %conv1d_result, %conv2d_result, %complex_result : 
           tensor<64xf32>, tensor<1x10x8x8xf32>, tensor<?x?x?x?x1xi32>, tensor<1x14x14x16xcomplex<f32>>
  }
}