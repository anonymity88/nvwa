module {
  ...
  func.func @decompose_static_concat_dim(%arg0: tensor<1x?x64xf32>, %arg1: tensor<1x?x64xf32>) -> tensor<1x?x128xf32> {
    %0 = tensor.concat dim(2) %arg0, %arg1 : (tensor<1x?x64xf32>, tensor<1x?x64xf32>) -> tensor<1x?x128xf32>
    return %0 : tensor<1x?x128xf32>
  }

  func.func @main(%src: tensor<4x1xf32>, %shape: tensor<2xi32>, %A: tensor<4x?xf32>, %source: tensor<1x2xf32>, %dest: tensor<4x4x4xf32>, %indices: tensor<1x2x3xi32>, %concat_arg0: tensor<1x?x64xf32>, %concat_arg1: tensor<1x?x64xf32>) -> (tensor<2x2xf32>, index, index, tensor<4x4x4xf32>, tensor<1x?x128xf32>) {
    ...
    %concatenated = call @decompose_static_concat_dim(%concat_arg0, %concat_arg1) : (tensor<1x?x64xf32>, tensor<1x?x64xf32>) -> tensor<1x?x128xf32>
    return %reshaped, %dim0, %dim1, %scattered, %concatenated : tensor<2x2xf32>, index, index, tensor<4x4x4xf32>, tensor<1x?x128xf32>
  }
}