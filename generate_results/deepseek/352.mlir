module {
  ...
  func.func @compute_and_log(%value: i32 {dialectName.argAttribute = "input"}, %scale: f32) -> (f32 {dialectName.resAttribute = "output"}) {
    ...
    %4 = call @external_compute(%value, %3) : (i32, f32) -> f32
    call @log_message() : () -> ()
    return %4 : f32
  }
  ...
  func.func @main() {
    ...
    %compute_log_result = call @compute_and_log(%arg_i32, %arg_f32) : (i32, f32) -> f32

    %doubled_value = call @double(%arg_i32) : (i32) -> i32
    ...
    %computed_result = call_indirect %func_handle_compute(%input_tensor, %scale_factor) 
    ...
    %tensor_sum = call_indirect %add_func(%tensor_a, %tensor_b) 
                   : (tensor<16xf32>, tensor<16xf32>) -> tensor<16xf32>

    // Call also_ite function
    %true = arith.constant true
    %then_val = arith.constant 1.0 : f32
    %else_val = arith.constant 0.0 : f32
    %ite_result = call @also_ite(%true, %then_val, %else_val) : (i1, f32, f32) -> f32
