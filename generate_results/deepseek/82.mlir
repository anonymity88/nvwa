module {
  ...
  func.func @main() {
    ...
    // Tensor add operations
    %tensor_a = arith.constant dense<1.0> : tensor<16xf32>
    %tensor_b = arith.constant dense<2.0> : tensor<16xf32>
    %add_tensors_func = func.constant @add_tensors : (tensor<16xf32>, tensor<16xf32>) -> tensor<16xf32>
    %tensor_result = func.call_indirect %add_tensors_func(%tensor_a, %tensor_b) 
